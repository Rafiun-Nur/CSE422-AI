# -*- coding: utf-8 -*-
"""Copy of 422 final.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1V_Y4p0-MFxIQl_9fIPJDGbzkkcEzlYa3
"""
...
for i, j in enumerate([lr, dt, nn]):
    if(hasattr(j, "predict_proba")):
        y_prob = j.predict_proba(X_test)
        for k in range(3):
            fpr, tpr, _ = roc_curve((y_test == k).astype(int), y_prob[:, k])
            auc = roc_auc_score((y_test == k).astype(int), y_prob[:, k])
            plt.plot(fpr, tpr,
                     label=f'{models[i]} - Class {["Low", "Medium", "High"][k]} (AUC = {auc:.2f})')
